...
# Twitter cookies from Chrome DevTools
AUTH_TOKEN = os.getenv("TWITTER_AUTH_TOKEN")
CT0 = os.getenv("TWITTER_CT0")

# By default, use a dummy value until the user sends their first message
YOUR_TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "123456789")

WATCHLIST_FILE = "watchlist.json"

# Default AI Twitter accounts if file doesn't exist
DEFAULT_ACCOUNTS = [
    "demishassabis",  # Demis Hassabis — Google DeepMind CEO
    "karpathy",       # Andrej Karpathy — ex-Tesla/OpenAI, AI educator
    "AnthropicAI",    # Anthropic — Claude makers
]
...
SUMMARY_TIME = "22:00"  # 10pm daily digest
...
def get_twitter_session():
    """Create a requests session that mimics a real browser logged into Twitter."""
...
# ---- FETCH TWEETS VIA GRAPHQL (same as Twitter website) ----
def get_user_id(session, username):
    """Get a user's Twitter ID via GraphQL - same endpoint the website uses."""
...
def get_user_tweets(session, user_id, count=5):
    """Get a user's tweets via GraphQL - same endpoint the website uses."""
        ...
        for instruction in timeline:
            ...
            for entry in entries:
                try:
                    ...
                    # Handle tweets with "tweet" wrapper
...
def fetch_tweets(username, count=5):
    """Fetch tweets for a user using the GraphQL API."""
    try:
        if not AUTH_TOKEN or not CT0:
            print(f"⚠️ No Twitter cookies in .env")
            return None
...
def main():
    ...
    t_sched = threading.Thread(target=run_scheduler, args=(app,), daemon=True)
    t_sched.start()

    print("🚀 Aryan is online!")
    print(f"📋 Watchlist: {', '.join(['@'+a for a in TWITTER_ACCOUNTS])}")
    print(f"🍪 Cookies loaded: auth_token={'✅' if AUTH_TOKEN else '❌'}, ct0={'✅' if CT0 else '❌'}")
    app.run_polling()
